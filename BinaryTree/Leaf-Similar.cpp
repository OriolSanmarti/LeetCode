...
class Solution {	
public:
	void addvector(TreeNode* root, vector<int>& vec){
        if(root == NULL) return;
        if(root->left == NULL and root->right == NULL) vec.push_back(root->val);
        addvector(root->left, vec);
        addvector(root->right, vec);
        return;
    }
...
int main(){
	...
	Solution sol;
	cout << sol.leafSimilar(root, root2) << endl;
}
